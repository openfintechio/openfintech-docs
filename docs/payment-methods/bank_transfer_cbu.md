
# Bank Transfer CBU 
![bank_transfer_cbu](https://static.openfintech.io/payment_methods/bank_transfer_cbu/logo.svg?w=400&c=v0.59.26#w200)  

## General 
**Code:** `bank_transfer_cbu` 
 
**Name:** 
 
:	[EN] Bank Transfer CBU 
 
**Category:** `bank_transfer` 
 

## Images 

### Logo 
![bank_transfer_cbu](https://static.openfintech.io/payment_methods/bank_transfer_cbu/logo.svg?w=400&c=v0.59.26#w200)  

```
https://static.openfintech.io/payment_methods/bank_transfer_cbu/logo.svg?w=400&c=v0.59.26#w200
```  

### Icon 
![bank_transfer_cbu](https://static.openfintech.io/payment_methods/bank_transfer_cbu/icon.svg?w=278&c=v0.59.26#w100)  

```
https://static.openfintech.io/payment_methods/bank_transfer_cbu/icon.svg?w=278&c=v0.59.26#w100
```  

## Payment Services 
 
The list of [Payment Services](/payment-services/) based on the _Bank Transfer CBU_ 

|Icon|Name|Code| 
|:---:|:---:|:---:| 
|![bank_transfer_cbu](https://static.openfintech.io/payment_methods/bank_transfer_cbu/icon.svg?w=278&c=v0.59.26#w100) |[bank_transfer_cbu_ars_hpp](/payment-services/bank_transfer_cbu_ars_hpp/)|`bank_transfer_cbu_ars_hpp`| 
|![bank_transfer_cbu](https://static.openfintech.io/payment_methods/bank_transfer_cbu/icon.svg?w=278&c=v0.59.26#w100) |[bank_transfer_cbu_ars_invoice](/payment-services/bank_transfer_cbu_ars_invoice/)|`bank_transfer_cbu_ars_invoice`| 
 

## Payment Providers 
 
The list of [Payment Providers](/payment-providers/) that support the _Bank Transfer CBU_ 

|Icon|Name|Code| 
|:---:|:---:|:---:| 
|![payplanet](https://static.openfintech.io/payment_providers/payplanet/icon.svg?w=278&c=v0.59.26#w100) |[PayPlanet](/payment-providers/payplanet/)|`payplanet`| 
|![safegatepayments](https://static.openfintech.io/payment_providers/safegatepayments/icon.png?w=278&c=v0.59.26#w100) |[SafeGate Payments](/payment-providers/safegatepayments/)|`safegatepayments`| 
 

## JSON Object 

```json
{
  "code":"bank_transfer_cbu",
  "vendor":null,
  "name":{
    "en":"Bank Transfer CBU"
  },
  "description":null,
  "countries":null,
  "category":"bank_transfer"
}
```  
