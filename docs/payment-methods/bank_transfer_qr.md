
# Bank Transfer QR 
![bank_transfer_qr](https://static.openfintech.io/payment_methods/bank_transfer_qr/logo.svg?w=400&c=v0.59.26#w200)  

## General 
**Code:** `bank_transfer_qr` 
 
**Name:** 
 
:	[EN] Bank Transfer QR 
 
**Category:** `bank_transfer` 
 

## Images 

### Logo 
![bank_transfer_qr](https://static.openfintech.io/payment_methods/bank_transfer_qr/logo.svg?w=400&c=v0.59.26#w200)  

```
https://static.openfintech.io/payment_methods/bank_transfer_qr/logo.svg?w=400&c=v0.59.26#w200
```  

### Icon 
![bank_transfer_qr](https://static.openfintech.io/payment_methods/bank_transfer_qr/icon.svg?w=278&c=v0.59.26#w100)  

```
https://static.openfintech.io/payment_methods/bank_transfer_qr/icon.svg?w=278&c=v0.59.26#w100
```  

## Payment Services 
 
The list of [Payment Services](/payment-services/) based on the _Bank Transfer QR_ 

|Icon|Name|Code| 
|:---:|:---:|:---:| 
|![bank_transfer_qr](https://static.openfintech.io/payment_methods/bank_transfer_qr/icon.svg?w=278&c=v0.59.26#w100) |[bank_transfer_qr_thb_hpp](/payment-services/bank_transfer_qr_thb_hpp/)|`bank_transfer_qr_thb_hpp`| 
|![bank_transfer_qr](https://static.openfintech.io/payment_methods/bank_transfer_qr/icon.svg?w=278&c=v0.59.26#w100) |[bank_transfer_qr_idr_hpp](/payment-services/bank_transfer_qr_idr_hpp/)|`bank_transfer_qr_idr_hpp`| 
 

## Payment Providers 
 
The list of [Payment Providers](/payment-providers/) that support the _Bank Transfer QR_ 

|Icon|Name|Code| 
|:---:|:---:|:---:| 
|![alikassa](https://static.openfintech.io/payment_providers/alikassa/icon.svg?w=278&c=v0.59.26#w100) |[AliKassa](/payment-providers/alikassa/)|`alikassa`| 
|![decard](https://static.openfintech.io/payment_providers/decard/icon.svg?w=278&c=v0.59.26#w100) |[Decard](/payment-providers/decard/)|`decard`| 
|![exxogate](https://static.openfintech.io/payment_providers/exxogate/icon.svg?w=278&c=v0.59.26#w100) |[Exxogate](/payment-providers/exxogate/)|`exxogate`| 
 

## JSON Object 

```json
{
  "code":"bank_transfer_qr",
  "vendor":null,
  "name":{
    "en":"Bank Transfer QR"
  },
  "description":null,
  "countries":null,
  "category":"bank_transfer"
}
```  
