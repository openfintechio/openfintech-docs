
# PayID 
![pay_id](https://static.openfintech.io/payment_methods/pay_id/logo.svg?w=400&c=v0.59.26#w200)  

## General 
**Code:** `pay_id` 
 
**Name:** 
 
:	[EN] PayID 
:	[RU] PayID 
:	[UK] PayID 
 
**Description:** 
 
: [EN] PayID is an Australian payment service that lets you link a bank account to an identifier such as an email, phone number or ABN. 
: [RU] PayID — австралийский платежный сервис, позволяющий привязать банковский счет к идентификатору, например email, номеру телефона или ABN. 
: [UK] PayID — австралійський платіжний сервіс, що дозволяє прив'язати банківський рахунок до ідентифікатора, наприклад email, номера телефону або ABN. 
 
**Category:** `alternative` 
 

## Images 

### Logo 
![pay_id](https://static.openfintech.io/payment_methods/pay_id/logo.svg?w=400&c=v0.59.26#w200)  

```
https://static.openfintech.io/payment_methods/pay_id/logo.svg?w=400&c=v0.59.26#w200
```  

### Icon 
![pay_id](https://static.openfintech.io/payment_methods/pay_id/icon.svg?w=278&c=v0.59.26#w100)  

```
https://static.openfintech.io/payment_methods/pay_id/icon.svg?w=278&c=v0.59.26#w100
```  

## Payout Services 
 
The list of [Payout Services](/payout-services/) based on the _PayID_ 

|Icon|Name|Code| 
|:---:|:---:|:---:| 
|![pay_id](https://static.openfintech.io/payout_methods/pay_id/icon.png?w=278&c=v0.59.26#w40) |[pay_id_aud](/payout-services/pay_id_aud/)|`pay_id_aud`| 
 

## Payment Providers 
 
The list of [Payment Providers](/payment-providers/) that support the _PayID_ 

|Icon|Name|Code| 
|:---:|:---:|:---:| 
|![unityfinance](https://static.openfintech.io/payment_providers/unityfinance/icon.svg?w=278&c=v0.59.26#w100) |[UnityFinance](/payment-providers/unityfinance/)|`unityfinance`| 
|![zepto](https://static.openfintech.io/payment_providers/zepto/icon.svg?w=278&c=v0.59.26#w100) |[Zepto](/payment-providers/zepto/)|`zepto`| 
|![impaya](https://static.openfintech.io/payment_providers/impaya/icon.png?w=278&c=v0.59.26#w100) |[Impaya](/payment-providers/impaya/)|`impaya`| 
|![finaxis](https://static.openfintech.io/payment_providers/finaxis/icon.png?w=278&c=v0.59.26#w100) |[FinAxis](/payment-providers/finaxis/)|`finaxis`| 
 

## JSON Object 

```json
{
  "code":"pay_id",
  "vendor":null,
  "name":{
    "en":"PayID",
    "ru":"PayID",
    "uk":"PayID"
  },
  "description":{
    "en":"PayID is an Australian payment service that lets you link a bank account to an identifier such as an email, phone number or ABN.",
    "ru":"PayID \u2014 \u0430\u0432\u0441\u0442\u0440\u0430\u043b\u0438\u0439\u0441\u043a\u0438\u0439 \u043f\u043b\u0430\u0442\u0435\u0436\u043d\u044b\u0439 \u0441\u0435\u0440\u0432\u0438\u0441, \u043f\u043e\u0437\u0432\u043e\u043b\u044f\u044e\u0449\u0438\u0439 \u043f\u0440\u0438\u0432\u044f\u0437\u0430\u0442\u044c \u0431\u0430\u043d\u043a\u043e\u0432\u0441\u043a\u0438\u0439 \u0441\u0447\u0435\u0442 \u043a \u0438\u0434\u0435\u043d\u0442\u0438\u0444\u0438\u043a\u0430\u0442\u043e\u0440\u0443, \u043d\u0430\u043f\u0440\u0438\u043c\u0435\u0440 email, \u043d\u043e\u043c\u0435\u0440\u0443 \u0442\u0435\u043b\u0435\u0444\u043e\u043d\u0430 \u0438\u043b\u0438 ABN.",
    "uk":"PayID \u2014 \u0430\u0432\u0441\u0442\u0440\u0430\u043b\u0456\u0439\u0441\u044c\u043a\u0438\u0439 \u043f\u043b\u0430\u0442\u0456\u0436\u043d\u0438\u0439 \u0441\u0435\u0440\u0432\u0456\u0441, \u0449\u043e \u0434\u043e\u0437\u0432\u043e\u043b\u044f\u0454 \u043f\u0440\u0438\u0432'\u044f\u0437\u0430\u0442\u0438 \u0431\u0430\u043d\u043a\u0456\u0432\u0441\u044c\u043a\u0438\u0439 \u0440\u0430\u0445\u0443\u043d\u043e\u043a \u0434\u043e \u0456\u0434\u0435\u043d\u0442\u0438\u0444\u0456\u043a\u0430\u0442\u043e\u0440\u0430, \u043d\u0430\u043f\u0440\u0438\u043a\u043b\u0430\u0434 email, \u043d\u043e\u043c\u0435\u0440\u0430 \u0442\u0435\u043b\u0435\u0444\u043e\u043d\u0443 \u0430\u0431\u043e ABN."
  },
  "countries":null,
  "category":"alternative"
}
```  
